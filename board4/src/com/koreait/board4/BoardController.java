...
public class BoardController {
	...
	public void list(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		param.setTyp(typ);
		request.setAttribute("jsList", new String[] {"board"}); // 자바스크립트를 추가
	...
	public void detail(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		//1. 아이피주소값 얻어야함
		//2. application에서 특정키값 만들어야함 즉 어떤 글인지 구분이 가능해야한다
		...
		String key = String.format("b_%d", i_board);
		
		ServletContext application = request.getServletContext(); // 객체 생성
		String valueip = (String) application.getAttribute(key);
		
		System.out.println(ip);
		System.out.println(key);
		System.out.println(valueip);
		if(valueip == null || !ip.equals(valueip)) {
	...
	public void del(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		String sql = " DELECT FROM t_board WHERE i_board = ? AND i_user = ? ";
